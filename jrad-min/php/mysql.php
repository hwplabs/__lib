...
{
    $sql_stmt = 'SELECT * FROM `' . $tb . '` ORDER BY id DESC';
    $result = $mysqli->query($sql_stmt);
    if ($result->num_rows > 0) {
        $new_array = array();
        while ($row = $result->fetch_assoc()) {
...
function sql_update_id($mysqli, $tb, $post_array, $id)
{
    ...
    // execute sql command
    $sql_stmt = 'UPDATE `' . $tb . '` SET ' . $hash_map . ' WHERE id="' . $mysqli->real_escape_string($id) . '"';
    $result = $mysqli->query($sql_stmt);
...
{
    $sql_stmt = 'DELETE FROM `' . $tb . '` WHERE id="' . $mysqli->real_escape_string($id) . '"';
    $result = $mysqli->query($sql_stmt);
    if ($result === true) {
        return $mysqli->affected_rows;
    } else {
...
function sql_fetch_by_date($mysqli, $tb, $where = 'DATE', $dt = null)
{
    $dt = is_null($dt) ? date('Y-m-d') : $dt;
    $sql_stmt = 'SELECT * FROM `' . $tb . '` WHERE DATE(' . $where . ')="' . $dt . '" ORDER BY id DESC';
    $result = sql_query($mysqli, $sql_stmt);
    return $result;
}
...
function sql_fetch_by_month($mysqli, $tb, $where = 'DATE', $mo = null, $yr = null)
{
    ...
    $yr = is_null($yr) ? date('Y') : $yr;
    $sql_stmt = 'SELECT * FROM `' . $tb . '` WHERE MONTH(' . $where . ')="' . $mo . '" AND YEAR(' . $where . ')="' . $yr . '" ORDER BY id DESC';
    $result = sql_query($mysqli, $sql_stmt);
    return $result;
}